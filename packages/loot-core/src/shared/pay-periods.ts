...
import { dayFromDate, parseDate } from '#shared/date-utils';
...
export const generatePayPeriods = memoizeOne(
  ...
  (newArgs, lastArgs) => {
    const [newYear, newConfig] = newArgs as [number, PayPeriodConfig | null];
    const [lastYear, lastConfig] = lastArgs as [number, PayPeriodConfig | null];
